...
The law atabase contains one row per law, with:
...
to turn this into a consistent, interpretable score, we used a simple symmetric scoring scheme:
  ...
  - Permissive law implemntation/modification -> -1
...
Why this makes sense:
  - The sign captures the direction of the change:
      ...
      - Negative values indicate teh environment becomes more permissive.
  - the magnitude (fixed a 1) assumes each law change contributes equally to the net policy climate. This is a simplification but has two advantages:
...
This avoids forcing ambiguous entries into one direction or the other.### Cumulative law strength by state and year
The key assumption is that once a law is implemented, its effect persists in all subsequent years until poentially offset by later changes. To refelct this, we:
...
Interpretation:
  - `law_strength_score` - new accumulated score:
    > (# restrictive implementation + permissive repeals) = (# permissive implementations + restrictive repeals)
...
```
  ...
  - Conceptual Example: `strength_background_checks` is positive is, overtime, background-check laws in the state have become more strictive than permissive, and megative is the reverse is true. 
  - Reason: this lets us disentabgle overall law strength into policy subdomains and analyze whether certain types of laws (e.g. background checks, concealed carry, castle doctrine) are more strongly associated with mortality patterns. 
...
To cpature how things change over time within a state, I computer differences:
